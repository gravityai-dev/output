 ...
 */
export async function publishForms(config: FormPublishConfig, api: any): Promise<{
  channel: string;
  success: boolean;
}> {
  const logger = api?.createLogger?.("FormPublisher") || console;
  ...
  try {
    ...
    if (!api || !api.gravityPublish) {
      throw new Error("API with gravityPublish not provided to publishForms");
    }
